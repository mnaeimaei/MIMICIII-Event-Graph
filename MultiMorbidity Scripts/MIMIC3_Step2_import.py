...
path_to_neo4j_import_directory = '/home/.config/Neo4j Desktop/Application/relate-data/dbmss/dbms-bd3c4134-088d-4bb9-8a7c-90b7e2ab43c6/import/'
...
model_entities =  [['Logistic', 'case', 'WHERE e.EventOrigin ="Logistic"'],
                   ...
                   ['Case_All', 'case', 'WHERE EXISTS(e.case)'],
                   ['Adm', 'HADM_ID', 'WHERE EXISTS(e.HADM_ID)']]
...
if step_ClearConstraints:
    #delete_constraints
    runQuery(driver, 'DROP CONSTRAINT ON (e:Event) ASSERT e.ID  IS UNIQUE;')
    runQuery(driver, 'DROP CONSTRAINT ON (en:Entity) ASSERT en.uID IS UNIQUE;')
    runQuery(driver, 'DROP CONSTRAINT ON (l:Log) ASSERT l.ID IS UNIQUE;')
    # table to measure performance
    perf = pd.DataFrame(columns=['name', 'start', 'end', 'duration'])
    start = time.time()
    last = start
...
if step_createEventAttrClasses:
    for relation in attr_relations:  # per relation
        ...
        perf = perf.append({'name':dataSet+'_create_other_classes', 'start':last, 'end':end, 'duration':(end - last)},ignore_index=True)
        print('Other classes done: took '+str(end - last)+' seconds')
        last = end
...
if step_createDFCWithoutFiltering: #Step14
    ...
    for entity in mylist: #per entity
        ...
        perf = perf.append(
            {'name': dataSet + '_aggregate_df_without_filtering' + entity, 'start': last, 'end': end, 'duration': (end - last)},
            ignore_index=True)
        print('Aggregating DF Without Filtering for ' + entity + ' done: took ' + str(end - last) + ' seconds')
        last = end
...
if step_createDFCFWithFiltering: #Step14
    ...
    for entity in mylist: #per entity
        ...
        perf = perf.append(
            {'name': dataSet + '_aggregate_df_wit_filtering' + entity, 'start': last, 'end': end, 'duration': (end - last)},
            ignore_index=True)
        print('Aggregating DF With Filtering for ' + entity + ' done: took ' + str(end - last) + ' seconds')
        last = end
